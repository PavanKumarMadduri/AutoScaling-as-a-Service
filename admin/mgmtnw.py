...
###############Create a persistent virtual network#################
    ...
    ssh.exec_command('ip link add '+tenant_name+'-link-dhcp type veth peer name dhcp-link-'+tenant_name)
    ssh.exec_command('ip link set dhcp-link-'+tenant_name+' up')
    ssh.exec_command('ip link set '+tenant_name+'-link-dhcp up')
    ssh.exec_command('brctl addif '+tenant_name+"-mgmt--br "+tenant_name+'-link-dhcp')
    ssh.exec_command('brctl addif '+tenant_name+"-"+'-dhcp-mgmt  dhcp-link-'+tenant_name)
